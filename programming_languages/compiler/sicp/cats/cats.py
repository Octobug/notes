...
def pick(paragraphs, select, k):
    ...
    while sl_i <= k:
        if pg_i >= len(paragraphs):
            return ''

        if select(paragraphs[pg_i]):
            if (sl_i == k):
                return paragraphs[pg_i]
            sl_i += 1
...
def about(subject):
    ...
    def is_word_in_subject(paragraph):
        words = split(paragraph)
        for w in words:
            if lower(remove_punctuation(w)) in subject:
                return True
        return False

    return is_word_in_subject
    # END PROBLEM 2
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for sw in word_list:
        diff = diff_function(typed_word, sw, limit)
        if diff < min_diff:
            min_diff = diff
            min_word = sw

    return typed_word if min_diff > limit else min_word

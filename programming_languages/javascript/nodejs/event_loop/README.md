...
It allows Node.js to perform non-blocking I/O operations by offloading
...
appropriate callback may be added to the **poll queue** to eventually be
executed.

### Event Loop Explained

When Node.js starts, it initializes the event loop, processes the provided
input script (or drops into the `REPL`) which may make async API calls,
schedule timers, or call `process.nextTick()`, then begins processing the event
loop.
...
- When the event loop enters a given phase, it will perform any operations
  specific to that phase, then execute callbacks in that phase's queue until
  the queue has been exhausted or the maximum number of callbacks has executed.
- When the queue has been exhausted or the callback limit is reached, the event
  loop will move to the next phase, and so on.

Since any of these operations may schedule more operations and new events
...
- **poll**:
  ...
  - execute I/O related callbacks (almost all with the exception of close
...
*may be executed* rather than the **exact** time a person wants it to be
executed.

- Timers callbacks will run as early as they can be scheduled after the
  specified amount of time has passed;
- however, Operating System scheduling or the running of other callbacks may
  delay them.
...
- When the event loop enters the **poll** phase, it has an empty queue
  ...
  remaining until the soonest ***timer's threshold*** is reached.
- While it is waiting 95 ms pass, `fs.readFile()` finishes reading the file and
  its callback which takes 10 ms to complete is added to the **poll queue** and
  executed.
- When the callback finishes, there are no more callbacks in the queue, so the
  event loop will see that the threshold of the soonest timer has been reached
  then wrap back to the **timers phase** to execute the timer's callback.

In this example, you will see that the total delay between the timer being
scheduled and its callback being executed will be 105ms.

> To prevent the poll phase from starving the event loop, `libuv` also has a
> hard maximum (system dependent) before it stops polling for more events.
...
*and there are no timers scheduled*, one of two things will happen:

- *If the **poll queue** is not empty*, the event loop will iterate through its
  queue of callbacks executing them **synchronously** until either the queue
...
- *If the **poll queue** is empty*, one of two more things will happen:
  - If scripts have been scheduled by `setImmediate()`, the event loop will end
    ...
    scheduled scripts.
  - If scripts have not been scheduled by `setImmediate()`, the event loop will
...
**timers** *whose time thresholds have been reached*. If one or more timers are ready, the event loop will wrap back to the timers phase to execute those timers' callbacks.

#### check

This phase allows a person to execute callbacks immediately after the
**poll phase** has completed. If the **poll phase** becomes idle and scripts
have been queued with `setImmediate()`, the event loop may continue to the
**check phase** rather than waiting.

`setImmediate()` **is actually a special timer that runs in a separate phase**
of the event loop. It uses a `libuv` API that schedules callbacks to execute
...
**poll phase** becomes idle, it will end and continue to the check phase rather than waiting for poll events.
...
then timing will be **bound by the performance of the process** (which can be
impacted by other applications running on the machine).
...
定时器检查：当定时器到期、有新的 I/O 事件发生或外部事件触发时，事件循环会中断空闲等待状态，并重新检查定时器。如果有定时器已经到期，事件循环会将定时器的回调函数添加到任务队列中。
重复流程：完成定时器检查后，事件循环会再次处理可能存在的 I/O 事件，并执行相应的回调函数。这个过程会不断重复，直到任务队列为空或达到某些指定的条件。

```mermaid
flowchart TD

timers["1. timers [queue]"] --> pc["2. pending callbacks [queue]"]
pc --> ip["3. idle, prepare [queue]"]
ip --> poll["4. poll [queue]"]

poll --> if_limit{If limit
is reached}

if_limit -->|yes| check
if_limit -->|no| if_empty{If poll queue
is empty}

poll --- chk_timer["If one or more
timers are ready,
add callbacks into
the timers queue."]

if_empty --> |no| iter[iterate queue]

iter--> if_empty

if_empty -->|yes| if_imme{"If scripts
have been
scheduled by
setImmediate()"}

if_empty ---|yes| chk_timer

if_imme -->|yes| check["5. check [queue]"]

if_imme -->|no| wait["wait for callbacks
to be added to
the queue, then
execute them
immediately"]
...
check --> cc["6. close callbacks [queue]"]
cc --> timers

macros["callbacks set by
setTimeout(),
setInterval()"] -.- timers
...
Any time you call `process.nextTick()` in a given phase, all callbacks passed
to `process.nextTick()` will be resolved ***before*** the event loop continues.
This can create some bad situations because
...
we guarantee that `apiCall()` always runs its callback ***after*** the rest of
the user's code and ***before*** the event loop is allowed to proceed.

To achieve this, the JS call stack is allowed to unwind then immediately
execute the provided callback which allows a person to make recursive calls to
`process.nextTick()` without reaching a
...
allow the script to run to completion. This allows the user to set any event
handlers they want.
...
> Every day more new modules are being added, which means every day we wait,
> more potential breakages occur. While they are confusing, the names
...
listening callback is placed in a `setImmediate()` (instead of in a
`process.nextTick()`). Unless a hostname is passed, binding to the port will
happen immediately. For the event loop to proceed, it must hit the
**poll phase**, which means there is a non-zero chance that a connection could
have been received allowing the connection event to be fired before the
listening event.

Another example is extending an `EventEmitter` and emitting an event from

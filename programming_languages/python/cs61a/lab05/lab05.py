...
def closer_city(lat, lon, city_a, city_b):
    ...
    distance_to_a = distance(fake_city, city_a)
    distance_to_b = distance(fake_city, city_b)
...
def count_palindromes(L):
    ...
    def is_palindrome(word):
        lo_word = word.lower()
        return lo_word == lo_word[::-1]

    return len(list(filter(lambda w: is_palindrome(w), L)))

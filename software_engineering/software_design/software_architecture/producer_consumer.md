...
- [生产者/消费者模式](#生产者消费者模式)
  ...
  - [双缓冲区](#双缓冲区)
    - [技术原理](#技术原理)
    - [几种状态](#几种状态)
      - [都在使用的状态（并发读写）](#都在使用的状态并发读写)
      - [单个缓冲区空闲的状态](#单个缓冲区空闲的状态)
      - [缓冲区的切换](#缓冲区的切换)
    - [可能的并发问题](#可能的并发问题)
      - [死锁](#死锁)
    - [双缓冲区的应用场景](#双缓冲区的应用场景)
...
当同步或互斥的开销非常明显的时候，就要考虑使用。

### 技术原理

使用两个缓冲区，一个用于生产者，另一个用于消费者。当俩缓冲区都操作完，就进行一次切换。
先前被生产者写入的转为消费者读出，先前消费者读取的转为生产者写入。
由于生产者和消费者不会同时操作同一个缓冲区，所以不需要在读写每一个数据单元时进行同步/互斥操作。

为了真正做到“不冲突”，还需要两个互斥锁 `La` 和 `Lb`，分别对应两个缓冲区。
...
设生产者写入 A，消费者读取 B。
在这种状态下，生产者拥有 `La`；消费者拥有锁 `Lb`。
由于两个缓冲区都处于独占状态，因此每次读写缓冲区中的元素都不需要再进行加锁、解锁操作。

#### 单个缓冲区空闲的状态

由于两个并发实体的速度会有差异，必然会出现一个缓冲区已经操作完，而另一个尚未操作完的情况。

假设生产者快于消费者。当生产者把 A 写满时，生产者要先释放 `La`，然后尝试获取 `Lb`。
由于 B 还没有被读空，`Lb` 还被消费者持有，所以生产者进入 `Suspend` 状态。

#### 缓冲区的切换

当消费者终于把 B 读完。这时，消费者也要先释放 `Lb`，然后尝试获取 `La`。
由于 `La` 刚才已经被生产者释放，所以消费者能立即拥有 `La` 并开始读取 A 的数据。
而由于 `Lb` 被消费者释放，所以生产者会 `Resume` 并拥有 `Lb`，然后生产者继续往 B 写入数据。

### 可能的并发问题

#### 死锁

假如把前面介绍的操作步骤调换顺序：生产者或消费者在操作完当前的缓冲区之后，先去获取另一个缓冲区的锁，再释放当前缓冲区的锁。

一旦两个并发实体同时处理完各自缓冲区，同时去获取对方拥有的锁，就会出现典型的死锁。
...
某些编程语言或者程序库提供了的线程安全的缓冲区。由于这种缓冲区会自动为每次读写进行同步/互斥，所以就把双缓冲的优势抵消掉了。因此，在使用双缓冲区时要避开这类缓冲区。

#### 用于并发进程

在进程间使用双缓冲，需要考察不同 IPC 类型的特点。
同样地，由于使用双缓冲的目的是降低同步/互斥的开销，对于那些已经封装了同步/互斥的 IPC 类型，就没必要做双缓冲了。

比较适合用双缓冲的主要是：共享内存和文件。
